...
#
# `asop-spec` is an ordinary release on PyPI, so pip resolves it from
# `[project] dependencies` like any other requirement. It used to be a uv
# workspace member under `packages/asop`, which pip could not resolve — hence
# the copy-and-install-from-tree dance that stood here. 42fcf7a moved the
# contract to its own repository and deleted `packages/`, and left these two
# lines pointing at the directory it had just removed.
