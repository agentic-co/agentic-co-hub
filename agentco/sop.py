...
The record contract — `SopStatus`, the `SOP` dataclass, `validate_fields`,
and the field-name constants that describe its shape — has moved to
`asop.sop`, the package shared with any harness that pins the same
`(sop_id, version)` off a work item (see `packages/asop/README.md`). This
module re-exports that contract and adds everything about KEEPING a
procedure that a bare record shape does not answer: the versioned store
(`SopLibrary` below), file locking, and the revision policy it enforces.
