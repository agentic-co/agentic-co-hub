...
def test_work_requiring_a_capability_is_invisible_to_a_worker_without_it(client):
    ...
    assert post(client, "/work/pull", "dana", {"capabilities": ["acme"]}).json()["state"] == "empty"
    ...
    pulled = post(client, "/work/pull", "kofi", {"capabilities": ["acme"]}).json()
    assert pulled["state"] == "leased"
    assert pulled["item"]["requires"] == ["acme"]
...
def test_an_sop_instance_carries_the_pin_and_the_capability(client):
    ...
    assert filed["requires"] == ["acme"]

    # And the worker that pulls it can read the procedure it is pinned to.
    pulled = post(client, "/work/pull", "kofi", {"capabilities": ["acme"]}).json()["item"]
