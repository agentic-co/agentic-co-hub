...
| **housekeeping** | expired leases, park clocks, quarantine, stranded runs — previewed, or run under `--apply` |
| **stranded steps** | step beads sitting ready, bound to an actor that has never pulled them, past that actor's cadence | **attention**. Every waiting step is reported as a row so a reader sees it before it is late; only the late ones are findings | none of these is a finding on its own: that is the system working. Gates abandoned past quarantine, and a queue resolving gates on the clock with no verdict behind any (`verifier_status`'s warning), are **attention** |
