...
| Revision policy | 🆕 | ✅ | Per STEP since v3. Protected tags freeze a step against agents ABSOLUTELY — an agent may not revise or activate a procedure containing one, deletion included, because changing what runs around a `money` step changes what that step does; class ratchets toward human only (role kind or gate kind), and removing a human step is the same demotion by deletion; on a FIRST activation the ratchet is absolute too, since baseline and target are the same version and every differential check would be vacuous; no undoing a human. A fourth rule, `human_only`, covers `retire` and `promote`. HTTP 403 `revision_policy:<rule>`, proven by mutation |
